...
static void write_pde_lz4(const char* path, int grid_x, int grid_y, int grid_z,
    ...
    const int ns = HCC::NUM_SUBSTRATES;
...
static void export_pde_async_no_join(int grid_x, int grid_y, int grid_z, const std::string& path) {
    ...
    HCC::g_pde_solver->get_all_concentrations_async(dst, g_pde_stream);
...
FLAMEGPU_STEP_FUNCTION(exportPDEData) {
    ...
    HCC::g_layer_timings.push_back({"io_pde_join",
        std::chrono::duration<double, std::milli>(t1 - t0).count()});
    HCC::g_layer_timings.push_back({"io_pde_export",
...
static void collect_ecm_to_buf(std::vector<float>& buf, int grid_x, int grid_y, int grid_z) {
    ...
    float* d_ecm = HCC::get_ecm_grid_device_ptr();
    float* d_fib = HCC::get_fib_density_field_device_ptr();
...
static void collect_abm_step0(flamegpu::CUDASimulation& sim,
    ...
    buf.clear();
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_CANCER_CELL));
    ...
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_TCELL));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i)
    ...
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_TREG));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i)
    ...
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_MDSC));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i)
    ...
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_MACROPHAGE));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i)
    ...
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_FIBROBLAST));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i) {
            ...
            const auto    sx    = p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_x");
            const auto    sy    = p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_y");
            const auto    sz    = p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_z");
            for (int s = 0; s < clen; ++s)
                abm_push(buf, ABM_TYPE_FIB, id, sx[s], sy[s], sz[s], state, life, 0);
        }
    }
    {
        flamegpu::AgentVector p(model.Agent(HCC::AGENT_VASCULAR));
        sim.getPopulationData(p);
        for (unsigned i = 0; i < p.size(); ++i) {
...
static void collect_abm_step(flamegpu::HostAPI* FLAMEGPU, std::vector<int32_t>& buf) {
    buf.clear();
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_CANCER_CELL).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i)
            abm_push(buf, ABM_TYPE_CANCER, (int32_t)p[i].getID(),
                p[i].getVariable<int>("x"), p[i].getVariable<int>("y"), p[i].getVariable<int>("z"),
                p[i].getVariable<int>("cell_state"), 0,
    ...
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_TCELL).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i)
            abm_push(buf, ABM_TYPE_TCELL, (int32_t)p[i].getID(),
                p[i].getVariable<int>("x"), p[i].getVariable<int>("y"), p[i].getVariable<int>("z"),
                p[i].getVariable<int>("cell_state"), p[i].getVariable<int>("life"), 0);
    ...
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_TREG).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i)
            abm_push(buf, ABM_TYPE_TREG, (int32_t)p[i].getID(),
                p[i].getVariable<int>("x"), p[i].getVariable<int>("y"), p[i].getVariable<int>("z"),
                p[i].getVariable<int>("cell_state"), p[i].getVariable<int>("life"), 0);
    ...
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_MDSC).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i)
            abm_push(buf, ABM_TYPE_MDSC, (int32_t)p[i].getID(),
                p[i].getVariable<int>("x"), p[i].getVariable<int>("y"), p[i].getVariable<int>("z"),
                0, p[i].getVariable<int>("life"), 0);
    ...
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_MACROPHAGE).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i)
            abm_push(buf, ABM_TYPE_MAC, (int32_t)p[i].getID(),
                p[i].getVariable<int>("x"), p[i].getVariable<int>("y"), p[i].getVariable<int>("z"),
                p[i].getVariable<int>("cell_state"), p[i].getVariable<int>("life"), 0);
    ...
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_FIBROBLAST).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i) {
            const int32_t id    = (int32_t)p[i].getID();
            const int     state = p[i].getVariable<int>("cell_state");
            const int     life  = p[i].getVariable<int>("life");
            ...
            for (int s = 0; s < clen; ++s)
                abm_push(buf, ABM_TYPE_FIB, id,
                    p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_x", s),
                    p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_y", s),
                    p[i].getVariable<int, HCC::MAX_FIB_CHAIN_LENGTH>("seg_z", s),
                    state, life, 0);
        }
    }
    {
        flamegpu::DeviceAgentVector p = FLAMEGPU->agent(HCC::AGENT_VASCULAR).getPopulationData();
        for (unsigned i = 0; i < p.size(); ++i) {
            int st = p[i].getVariable<int>("cell_state");
            if (st == 1) continue;
            abm_push(buf, ABM_TYPE_VAS, (int32_t)p[i].getID(),
...
FLAMEGPU_STEP_FUNCTION(exportABMData) {
    ...
    HCC::g_layer_timings.push_back({"io_abm_join", join_ms});
    HCC::g_layer_timings.push_back({"io_abm_collect", collect_ms});
}
...
FLAMEGPU_STEP_FUNCTION(stepCounter) {
    ...
    // Agent counts
    const unsigned int cancer_count = FLAMEGPU->agent(HCC::AGENT_CANCER_CELL).count();
    const unsigned int tcell_count  = FLAMEGPU->agent(HCC::AGENT_TCELL).count();
    const unsigned int treg_count   = FLAMEGPU->agent(HCC::AGENT_TREG).count();
    const unsigned int mdsc_count   = FLAMEGPU->agent(HCC::AGENT_MDSC).count();
    const unsigned int mac_count   = FLAMEGPU->agent(HCC::AGENT_MACROPHAGE).count();
    const unsigned int fib_count   = FLAMEGPU->agent(HCC::AGENT_FIBROBLAST).count();
    const unsigned int vas_count   = FLAMEGPU->agent(HCC::AGENT_VASCULAR).count();
...
int main(int argc, const char** argv) {
    ...
    HCC::LymphCentralWrapper _lymph;
    _lymph.initialize(param_file);
    HCC::set_internal_params(*model, _lymph);
    HCC::set_lymph_pointer(&_lymph);  // Set global pointer for QSP host functions
    init_lap("init_qsp");
    ...
    cudaMalloc(&device_event_counters, HCC::ABM_EVENT_COUNTER_SIZE * sizeof(unsigned int));
    cudaMalloc(&device_state_counters, HCC::ABM_STATE_COUNTER_SIZE * sizeof(unsigned int));
    ...
    HCC::initializeToQSP(simulation, *model, config, _lymph);
    ...
    HCC::set_qsp_output_path(qsp_seed_path);
    ...
    unsigned int init_states[HCC::ABM_STATE_COUNTER_SIZE] = {};
    {
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_CANCER_CELL));
            ...
            for (unsigned i = 0; i < p.size(); ++i) {
                ...
                if      (s == HCC::CANCER_STEM)       init_states[HCC::SC_CANCER_STEM]++;
                else if (s == HCC::CANCER_PROGENITOR) init_states[HCC::SC_CANCER_PROG]++;
                else                                   init_states[HCC::SC_CANCER_SEN]++;
            }
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_TCELL));
            ...
            for (unsigned i = 0; i < p.size(); ++i) {
                int s = p[i].getVariable<int>("cell_state");
                if      (s == HCC::T_CELL_EFF) init_states[HCC::SC_CD8_EFF]++;
                else if (s == HCC::T_CELL_CYT) init_states[HCC::SC_CD8_CYT]++;
                else                            init_states[HCC::SC_CD8_SUP]++;
            }
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_TREG));
            ...
            for (unsigned i = 0; i < p.size(); ++i) {
                int s = p[i].getVariable<int>("cell_state");
                if (s == HCC::TCD4_TH) init_states[HCC::SC_TH]++;
                else                    init_states[HCC::SC_TREG]++;
            }
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_MDSC));
            simulation.getPopulationData(p);
            init_states[HCC::SC_MDSC] = p.size();
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_MACROPHAGE));
            simulation.getPopulationData(p);
            for (unsigned i = 0; i < p.size(); ++i) {
                int s = p[i].getVariable<int>("cell_state");
                if (s == HCC::MAC_M1) init_states[HCC::SC_MAC_M1]++;
                else                   init_states[HCC::SC_MAC_M2]++;
            }
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_FIBROBLAST));
            ...
            for (unsigned i = 0; i < p.size(); ++i) {
                ...
                if (s == HCC::FIB_NORMAL) init_states[HCC::SC_FIB_NORM] += clen;
                else                       init_states[HCC::SC_FIB_CAF]  += clen;
            }
        }
        {
            flamegpu::AgentVector p(model->Agent(HCC::AGENT_VASCULAR));
            ...
            for (unsigned i = 0; i < p.size(); ++i) {
                int s = p[i].getVariable<int>("cell_state");
                if (s == HCC::VAS_TIP) init_states[HCC::SC_VAS_TIP]++;
                else                    init_states[HCC::SC_VAS_PHALANX]++;
            }
        }
    }
    ...
    if (stats_file.is_open()) {
        ...
        stats_file << "0,"
            << init_states[HCC::SC_CANCER_STEM] << "," << init_states[HCC::SC_CANCER_PROG] << "," << init_states[HCC::SC_CANCER_SEN] << ","
            << init_states[HCC::SC_CD8_EFF]     << "," << init_states[HCC::SC_CD8_CYT]     << "," << init_states[HCC::SC_CD8_SUP]   << ","
            << init_states[HCC::SC_TH]          << "," << init_states[HCC::SC_TREG]         << ","
            << init_states[HCC::SC_MDSC]        << ","
            << init_states[HCC::SC_MAC_M1]      << "," << init_states[HCC::SC_MAC_M2]       << ","
            << init_states[HCC::SC_FIB_NORM]    << "," << init_states[HCC::SC_FIB_CAF]      << ","
            << init_states[HCC::SC_VAS_TIP]     << "," << init_states[HCC::SC_VAS_PHALANX]  << ","
    ...
    for (unsigned int i = 0; i < total_steps; i++) {
        ...
        double pde_ms = HCC::get_last_pde_ms();
        double qsp_ms = HCC::get_last_qsp_ms();
        ...
        // Write per-layer timings collected by checkpoint host functions
        {
            ...
            for (const auto& lt : HCC::g_layer_timings) {
                layer_file << i << "," << lt.name << "," << lt.ms << "\n";
            }
            HCC::g_layer_timings.clear();
    ...
    flamegpu::AgentVector final_cancer(model->Agent(HCC::AGENT_CANCER_CELL));
    flamegpu::AgentVector final_tcells(model->Agent(HCC::AGENT_TCELL));
    flamegpu::AgentVector final_tregs(model->Agent(HCC::AGENT_TREG));
    flamegpu::AgentVector final_mdscs(model->Agent(HCC::AGENT_MDSC));
    flamegpu::AgentVector final_macs(model->Agent(HCC::AGENT_MACROPHAGE));
    flamegpu::AgentVector final_fibs(model->Agent(HCC::AGENT_FIBROBLAST));
    ...
    if (final_tcells.size() > 0) {
        ...
        for (unsigned int i = 0; i < final_tcells.size(); i++) {
            ...
            if (state == HCC::T_CELL_EFF) eff_count++;
            else if (state == HCC::T_CELL_CYT) cyt_count++;
            else if (state == HCC::T_CELL_SUPP) supp_count++;
        }
