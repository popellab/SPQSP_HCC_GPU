...
// QSP CSV export step function — defined at file scope (not inside HCC namespace)
// so it matches the linkage of exportPDEData/exportABMData/stepCounter in main.cu.
// g_lymph and g_qsp_csv are static in HCC namespace in the same TU, so accessible here.
